...
def map(to_map,subsets,bflag):
	...
	for i in range(len(to_map)):
		...
		if (to_map.at[i, 'chr'] in subsets[0]):
			...
			#    tss.append(subsets[1][chrom_index].at[closest_feature_index, 'start_point']) #! remove later
...
parser.add_argument('-i', '--index', default='none', help="This represents the go index, a merge of the gene ontology annotation and ontology from geneontology.org. This can be created with the create_go_index.py script")
...
parser.add_argument('--phenotype', default='none', help="add filepath to phenotype data and annotate peaks. Tab seperated text or comma seperated are both acceptable. file should contain label named consistently with --feature class tag (symbol or ensg) and phenotype values colum named epsilon")
